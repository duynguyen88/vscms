...
{	
	...
	function generateAction() 
	{
		...
		else
		{
			...
			foreach($columnData as $columnDetail)
			{
				...
				$formData['flabel'][$columnDetail['Field']] = ucfirst($propValue);
	...
	private function getSearchReplaceInfo($formData)
	{
		...
		{
			$tmp .= '$this->' . $v . ' = $row[\''.$k.'\'];' . "\n\t\t";
			$tmp2 .= '$my'.$formData['fmodule'].'->' . $v . ' = $row[\''.$k.'\'];' . "\n\t\t\t";
		}
		...
		foreach($formData['flabel'] as $k => $v)
		{
			...
			{	
				...
				{
					$modifier = "|date_format:\$lang.default.dateFormatTimeSmarty";
				}
				else
				{
					$modifier = "";
